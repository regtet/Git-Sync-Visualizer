...
- **多分支同步**：选择源分支和多个目标分支，一键触发同步。
- **三种同步策略**
  - `分支合并`：顺序执行 `git fetch → checkout → merge --no-edit → push`。
  - `精准提交`：对指定 commit 哈希执行 `git cherry-pick`，适合补丁式同步。
  - `同步储藏`：选取 `git stash` 记录，自动应用、提交（使用储藏备注作为提交信息）并推送。
- **多仓库支持**：主界面顶部可随时切换工作目录，每次切换自动刷新仓库信息、分支列表与储藏记录。
- **实时日志**：右侧日志面板滚动展示各个步骤的执行结果，可一键清空。
- **主题切换**：内置“深空 / 极光”两套主题，一键切换，适应不同偏好。
- **通知系统**：顶部弹出式提示展示同步成败、异常信息。
- **储藏备注识别**：显示与提交时优先采用自定义的储藏信息，避免冗长默认提示。
